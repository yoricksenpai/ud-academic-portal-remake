        ...
        {notifications.map((notification) => (
          ...
          >
            <div className="flex items-start gap-2">
              {!notification.read && <div className="h-2 w-2 rounded-full bg-blue-500 mt-1.5 flex-shrink-0" />}
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  <div className="flex items-center">
                    {getNotificationIcon(notification.type)}
                    <h4 className="text-sm font-medium ml-1">{notification.title}</h4>
                  </div>
                  <span className="text-xs text-muted-foreground">{formatDate(notification.createdAt)}</span>
                </div>
                <p className={`text-xs mt-1 ${expandedId === notification.id ? "" : "line-clamp-2"}`}>
